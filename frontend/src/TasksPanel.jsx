...
export default function TasksPanel({ request, username, onBack }) {
  ...
  return <main className="App tasks-panel">
    <header><h1>Aufgaben</h1><p>Gemeinsam organisieren, Zuständigkeiten festhalten und abhaken.</p></header>
    <div className="task-toolbar">
      <button type="button" className="btn-edit" disabled={busy} onClick={onBack}>Zum Startmenü</button>
      <button type="button" className="btn-save-players" disabled={busy || loading || !!draft} onClick={() => { setDraft(empty()); setEditing(null); }}>Neue Aufgabe</button>
    ...
    <div className="task-toolbar" aria-label="Aufgaben filtern">
      <button type="button" className="btn-edit" aria-pressed={!mine} onClick={() => setMine(false)}>Alle</button>
      <button type="button" className="btn-edit" aria-pressed={mine} onClick={() => setMine(true)}>Meine Aufgaben</button>
      <span>{shown.filter(task => !task.completed).length} offen, {shown.filter(task => task.completed).length} erledigt</span>
    </div>
    {loading ? <p role="status">Aufgaben werden geladen…</p> : !shown.length ? <p>{mine ? 'Dir sind noch keine Aufgaben zugewiesen.' : 'Noch keine Aufgaben. Lege eure erste Aufgabe an.'}</p> :
      <ul className="task-list">{shown.map(task => <li key={task._id} className={`task-card${isOverdue(task) ? ' task-overdue' : ''}${task.completed ? ' task-completed' : ''}`}>
        ...
        <button type="button" className="btn-edit" disabled={busy || !!draft} onClick={() => { setEditing(task._id); setDraft({ title: task.title, description: task.description, dueDate: task.dueDate, assignedTo: task.assignedTo }); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>Bearbeiten</button></div>
      </li>)}</ul>}
  </main>;
}
