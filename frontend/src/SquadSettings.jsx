...
const empty = { name: '', playerId: '', guest: true, foot: 'unbekannt', mainPosition: '', positions: [], number: '', club: '', note: '', inactive: false };
export default function SquadSettings({ request, onProfileSaved }) {
  ...
  async function call(path, method, body) {
    const res = await request(path, method ? { method, headers: { 'Content-Type': 'application/json' }, body: body === undefined ? undefined : JSON.stringify(body) } : { cache: 'no-store' });
  ...
  const edit = p => setDraft({ ...empty, ...p, playerId: p.playerId || '', positions: p.positions || [] });
  return <section className="squad-settings">
    ...
    {data && <>
      ...
      {draft && <form className="cash-entry-section squad-fields" onSubmit={e => { e.preventDefault(); run(async () => { setData(await call('squads/profiles', 'POST', { ...draft, version: data.version })); setDraft(null); setNotice('Profil gespeichert.'); await onProfileSaved?.(); }); }}>
        <h3>{draft.guest ? 'Gastspielerin' : 'Spielerinnenprofil'}</h3>
        <label>Name<input required maxLength={100} value={draft.name} disabled={busy || (!!draft.playerId && !draft.guest)} onChange={e => setDraft({ ...draft, name: e.target.value })} /></label>
        ...
        {draft.guest && <label><input type="checkbox" checked={draft.inactive} disabled={busy} onChange={e => setDraft({ ...draft, inactive: e.target.checked })} />Gastspielerin inaktiv</label>}
        <div className="task-toolbar"><button className="btn-save-players" disabled={busy}>Profil speichern</button><button type="button" className="btn-edit" disabled={busy} onClick={() => setDraft(null)}>Abbrechen</button>{draft.guest && draft.playerId && !draft.legacyGuest && <button type="button" className="btn-delete" disabled={busy} onClick={() => { if (!window.confirm(`${draft.name} wirklich als Gastspielerin löschen?`)) return; run(async () => { setData(await call(`squads/guests/${draft.playerId}`, 'DELETE')); setDraft(null); setNotice('Gastspielerin gelöscht.'); await onProfileSaved?.(); }); }}>Gastspielerin löschen</button>}</div>
      </form>}
