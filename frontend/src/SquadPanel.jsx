...
import { POSITIONS, slots, suggest, movePlayer, score } from './squadUtils.js';
const dateInput = d => new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 10);
export default function SquadPanel({ request, onBack }) {
  const [data, setData] = useState(null), [draft, setDraft] = useState(null), [dirty, setDirty] = useState(false);
  const [busy, setBusy] = useState(false), [error, setError] = useState(''), [notice, setNotice] = useState('');
  const [stats, setStats] = useState([]), [excluded, setExcluded] = useState([]), [selected, setSelected] = useState('');
  ...
  function open(game) { if (!leave()) return; const now = new Date(), from = new Date(); from.setDate(from.getDate() - 56);
    setDraft(game ? structuredClone(game) : { opponent: '', location: '', date: dateInput(now), time: '10:00', from: dateInput(from), to: dateInput(now), fieldPlayers: data.fieldPlayers, benchSize: data.benchSize, formation: data.formation, lineup: [], captainId: '' });
    setDirty(false); setStats([]); setExcluded([]); setSelected(''); setError(''); setNotice('');
  }
  ...
  function add(p, role) {
    ...
    change({ lineup: [...draft.lineup, { ...spot, personId: p.id, name: p.name, guest: p.guest, role }] }); setError('');
  }
  ...
  return <div className="App squad-panel"><SquadModuleBrand /><div className="squad-heading"><h1>Spielkader</h1><button className="btn-back" disabled={busy} onClick={() => { if (leave()) onBack(); }}>Zurück</button></div>
    ...
    {draft && <section className="squad-editor">
      <form className="squad-fields" onSubmit={e => { e.preventDefault(); run(async () => { const result = await call('squads/games', { ...draft, id: draft._id, version: data.version }); setData(result); setDraft(result.games.find(g => g._id === draft._id) || result.games.at(-1)); setDirty(false); setNotice('Spiel und Aufstellung gespeichert.'); }); }}>
        ...
        <div className="squad-wide"><strong>{draft.fieldPlayers}+1 · Formation {draft.formation} · {draft.benchSize} Ersatzplätze</strong><p>Voreinstellungen für neue Spiele verwaltet ein Admin in den Einstellungen.</p></div>
        ...
        <label>Kapitänin<select disabled={busy} value={draft.captainId} onChange={e => change({ captainId: e.target.value })}><option value="">Noch offen</option>{draft.lineup.map(p => <option key={p.personId} value={p.personId}>{p.name}</option>)}</select></label>
      ...
      <div className="squad-layout"><section><h2>Aufstellung · {count('field') + count('keeper')}/{draft.fieldPlayers + 1}</h2><p className="squad-help">Spielerin ziehen oder antippen und anschließend ihre Zielposition antippen. Mit den Pfeiltasten kannst du sie ebenfalls verschieben.</p>
        <div ref={pitch} className="squad-pitch" aria-label="Grafische Aufstellung" onClick={e => { if (!busy && selected) move(selected, e); }}>
          ...
          {draft.lineup.filter(p => p.role !== 'bench').map(p => <button type="button" key={p.personId} disabled={busy} aria-label={`${p.name}, ${POSITIONS[p.position] || 'Feld'}, mit Pfeiltasten verschieben`} aria-pressed={selected === p.personId} className={`pitch-player ${p.role} ${selected === p.personId ? 'selected' : ''}`} style={{ left: `${p.x}%`, top: `${p.y}%` }}
            ...
            <span className="player-shirt">{data.candidates.find(c => c.id === p.personId)?.number || p.position || '·'}{draft.captainId === p.personId && <b>C</b>}</span><span className="player-name">{p.name}{p.guest ? ' · Gast' : ''}</span></button>)}
        </div><h3>Ersatzbank · {count('bench')}/{draft.benchSize}</h3><div className="squad-bench">{draft.lineup.filter(p => p.role === 'bench').map(p => <span key={p.personId}>{p.name}{p.guest ? ' · Gast' : ''}{draft.captainId === p.personId ? ' · C' : ''}</span>)}{!count('bench') && <p>Noch unbesetzt</p>}</div>
        <div className="squad-roster">{draft.lineup.map(p => <div key={p.personId} className="squad-roster-row"><strong>{p.name}{p.guest ? ' · Gast' : ''}</strong><label>Bereich<select disabled={busy} value={p.role} onChange={e => roleChange(p, e.target.value)}><option value="keeper">Tor</option><option value="field">Feld</option><option value="bench">Ersatzbank</option></select></label><label>Position<select disabled={busy || p.role === 'keeper'} value={p.position} onChange={e => change({ lineup: draft.lineup.map(row => row.personId === p.personId ? { ...row, position: e.target.value } : row) })}><option value="">Offen</option>{Object.entries(POSITIONS).map(([id, name]) => <option key={id} value={id}>{id} · {name}</option>)}</select></label><button className="btn-edit" disabled={busy} onClick={() => { change({ lineup: draft.lineup.filter(row => row.personId !== p.personId), captainId: draft.captainId === p.personId ? '' : draft.captainId }); if (selected === p.personId) setSelected(''); }}>Entfernen</button></div>)}</div>
      </section><section><h2>Verfügbare Spielerinnen</h2><p>Häkchen bestimmen, wer beim Vorschlag berücksichtigt wird.</p>{data.candidates.filter(p => !p.inactive).map(p => { const s = stats.find(s => s.id === p.id), inTeam = draft.lineup.some(row => row.personId === p.id); return <article className="squad-candidate" key={p.id}><strong>{p.name}{p.guest ? ' · Gast' : ''}</strong><small>{POSITIONS[p.mainPosition] || 'Stammposition offen'} · Fuß: {p.foot || 'unbekannt'}{p.positions?.length ? ` · Auch: ${p.positions.join(', ')}` : ''}{p.club ? ` · ${p.club}` : ''}</small>{p.note && <small>{p.note}</small>}{!p.guest && <label><input type="checkbox" disabled={busy} checked={!excluded.includes(p.id)} onChange={e => setExcluded(e.target.checked ? excluded.filter(id => id !== p.id) : [...excluded, p.id])} />Beim Vorschlag berücksichtigen</label>}{s && <small>{s.total ? `${s.attended}/${s.total} Trainings · ${Math.round(s.attendance * 100)} % · Sterne: ${s.average === null ? 'unbewertet' : s.average.toFixed(1)} · Wert ${score(s)}/100` : 'Keine Trainingsdaten im Zeitraum'}</small>}<div className="task-toolbar">{inTeam ? <span>Im Kader</span> : [['keeper', 'Tor'], ['field', 'Feld'], ['bench', 'Bank']].map(([role, title]) => <button className="btn-edit" key={role} disabled={busy} onClick={() => add(p, role)}>＋ {title}</button>)}</div></article>; })}</section></div>
