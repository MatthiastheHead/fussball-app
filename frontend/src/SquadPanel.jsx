...
export default function SquadPanel({ request, onBack, username }) {
  ...
  function add(p, role) {
    const availableIds = draft.availableIds.includes(p.id) ? draft.availableIds : [...draft.availableIds, p.id];
    ...
    change({ availableIds, lineup, ...leadersFor(lineup) }); setError('');
  }
