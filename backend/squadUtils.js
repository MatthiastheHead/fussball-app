...
function game(input, candidates) {
  ...
  const lineup = input.lineup.map(row => {
    ...
    if (!/^[pg]:[a-f\d]{24}$/i.test(row.personId) || !candidate || typeof candidate.name !== 'string' || !candidate.name.trim() || candidate.name.length > 100 || candidate.guest !== row.personId.startsWith('g:') || candidate.inactive || seen.has(row.personId)) fail('Unbekannte, inaktive oder doppelte Spielerin im Kader.');
  ...
  const captainId = input.captainId || '';
  if (captainId && !seen.has(captainId)) fail('Die Kapitänin muss im Kader sein.');
  return { ...config, opponent, location, date: input.date, time: input.time, from: input.from, to: input.to, lineup, captainId };
}
module.exports = { POSITIONS, validDate, settings, profile, game };
